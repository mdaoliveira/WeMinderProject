...
export const getTasks = (req, res) => {
  ...
  db.query(qSimpleTasks, (errSimple, simpleTasks) => {
    if (errSimple) {
      ...
      return res
        .status(500)
        .json({ error: "Erro de servidor!", details: errSimple.message });
    }
  ...
  if (is_complex) {
    ...
    db.query(qTask, values, (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Erro de servidor!", details: err.message });
      }
      ...
      db.query(qSubtask, [subtaskValues], (err2) => {
        if (err2) {
          ...
          return res
            .status(500)
            .json({ error: "Erro de servidor!", details: err2.message });
        }
        return res.status(201).json(createdTask);
      });
    });
...
export const editTask = (req, res) => {
  const taskId = req.params.id;
  const { title, description, priority, due_date, is_completed, subtarefas } =
    req.body;
  ...
  db.query(checkSimpleQ, [taskId], (errSimple, simpleResult) => {
    if (errSimple) {
      ...
      return res
        .status(500)
        .json({ message: "Erro de servidor!", error: errSimple.message });
    }
